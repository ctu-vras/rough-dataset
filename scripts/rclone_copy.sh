...
rclone copy -P \
  ...
  --filter '+ /*/*/*caminfo.*.yaml' --filter '+ /*/*/*calib.yaml' --filter '+ /*/*/*transformations.yaml' --filter '+ /*/*/weak_unsafe.csv' \
  ...
  --filter '+ /calib/**' --filter '+ /code/**' \
  --filter '- **' \
  --drive-chunk-size 512M --fast-list --transfers=6 --checkers=6 --buffer-size=75M -L \
  . gdrive:ROUGH/